...
export default function Unlock({ onUnlock }: UnlockProps) {
  ...
  const navigateAfterUnlock = async () => {
    ...
    // Check for pending validator registration deep link
    const pendingValidator = sessionStorage.getItem('sultan_pending_validator');
    if (pendingValidator) {
      sessionStorage.removeItem('sultan_pending_validator');
      navigate(pendingValidator);
      return;
    }
